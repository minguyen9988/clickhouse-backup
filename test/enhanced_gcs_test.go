...
func (m *MockEnhancedGCS) DeleteBatch(ctx context.Context, keys []string) (*enhanced.BatchResult, error) {
				...
				default:
					...
					if m.clientPool.shouldFail {
						...
						if shouldFailThisKey {
							...
							case "permission_denied", "auth_error":
	...
	m.clientPool.retryAttempts += totalRetryAttempts
	if m.clientPool.shouldFail && totalRetryAttempts == 0 {
